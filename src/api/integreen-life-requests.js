...
export async function request_plug_details({ bz, outlets }) {
  try {
    const url = fetch_url("flat/EChargingStation,EChargingPlug", "scode,smetadata.outlets", "sactive.eq.true", bz);
    const request = await fetch(url, fetch_options);
    const response = await request.json();
    if (outlets) {
...
export async function request_plug_types(bz) {
  try {
    const url = fetch_url("flat/EChargingPlug", "smetadata.outlets.0.outletTypeCode", "sactive.eq.true,smetadata.outlets.0.outletTypeCode.neq.UNKNOWN", bz);
    const request = await fetch(url, fetch_options);
    const response = await request.json();
...
export async function request_station_states(bz) {
  try {
    const url = fetch_url("flat/EChargingPlug/echarging-plug-status/latest", "pcode,pmetadata.state,mvalue,smetadata.outlets", "sactive.eq.true,pactive.eq.true", bz);
    const request = await fetch(url, fetch_options);
    const response = await request.json();
    return response.data;
  } catch (e) {
